...
if(empty($_POST["oldemail"])) {
    ...
    echo '<script>alert("Must provide old email"); window.location.href="/emailChangeProvider.php"  ;</script>';
    //Need to do exit(), as otherwise the php code will keep running even if the user goes to another page.
    exit();
}
if(empty($_POST["newemail"])) {
    // saying to print in the .php or the .html file what follows. If it's script, it'll excecute it.
    echo '<script>alert("Must provide new email"); window.location.href="/emailChangeProvider.php";</script>';
    //Need to do exit(), as otherwise the php code will keep running even if the user goes to another page.
    exit();
}
...
if($oldemail == $_SESSION['Email']) {
    ...
    if ($stmt = $mysqli->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: /emailChangeProvider.php?success=" . urlencode($successMessage));
            exit();
        } else {
            $errorMessage = "Error updating record: " . $mysqli->error;
            header("Location: /emailChangeProvider.php?error=" . urlencode($errorMessage));
            exit();
        }
        $stmt->close();
    } else {
