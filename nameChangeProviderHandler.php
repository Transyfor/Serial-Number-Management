...
if(empty($_POST["newname"])) {
    ...
    echo '<script>alert("Must provide new name"); window.location.href="/nameChangeProvider.php";</script>';
...
if ($stmt = $mysqli->prepare($sql)) {
    ...
    if ($stmt->execute()) {
        ...
        header("Location: /nameChangeProvider.php?success=" . urlencode($successMessage));
        exit();
    } else {
        $errorMessage = "Error updating record: " . $mysqli->error;
        header("Location: /nameChangeProvider.php?error=" . urlencode($errorMessage));
        exit();
    }
    $stmt->close();
} else {
